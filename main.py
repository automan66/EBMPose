...
import time
from model_nce import EBMPrior
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Path to the pretrained EBM checkpoint."
)
...
parser.add_argument(
    ...
    type=int,
    default=10,
    help="Number of gradient refinement steps."
)
...
netE = EBMPrior().cuda()
netE.load_state_dict(torch.load(args.ckpt))
netE.eval() # important  
...
def step(split, args, actions, dataLoader, model, optimizer=None, epoch=None):
    ...
    for i, data in enumerate(tqdm(dataLoader, 0)):
        ...
        if split == 'train':
            ...
            x0 = xfeat.squeeze().view(-1, 17*512) #[1, 17*512]
            y0 = output_3D.squeeze().view(-1, 17*3) #[1, 17*3]
            y0 = set_root_zero(y0)

            x = Variable(x0.clone().detach(), requires_grad=True)
            y = Variable(y0.clone().detach(), requires_grad=True)
            ...
            for _ in range(args.ref_step):
                y_grad = torch.autograd.grad(netE(x, y_best).sum(), [y_best], retain_graph=True)[0]
                y = y_best - alpha*y_grad
            ...
            output_3D = y_best.view(-1, 17, 3).clone().detach().unsqueeze(1)
...
if __name__ == '__main__':
    ...
    if args.test:
        ...
        test_dataloader = torch.utils.data.DataLoader(test_data, batch_size=1, #args.batch_size,
